...
### Наблюдение по продуктовой логике

В таблицу «Алерты» попадают события трёх уровней — `info`/`warning`/`critical`. На каждое штатно обработанное файло создаётся `info: File processed successfully`, то есть лента засоряется успехами. По смыслу слова «алерт» туда должны попадать только `warning`/`critical`, а успехи — в отдельный audit-лог или вовсе не писаться. Я не стал трогать бизнес логику.
